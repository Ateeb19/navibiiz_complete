...
const Groupage = () => {
    ...
    const filteredCompanies = company_info.filter((item) => item.groupage_service === "1");
    ...
    const filteredAndSearchedCompanies = filteredCompanies.filter((company) =>
        company.Countries.some(
            (country) =>
                ...
                (date === "" || new Date(country.date) >= new Date(date))
        )
    );
    return (
        <div>
            ...
            <h2 className="text-uppercase">groupage</h2>
            <div>
                {
                    mode === 'offline' ?
            ...
            <div className="d-flex flex-column justify-content-center align-items-center ">
                ...
                <div className="mt-4 d-flex flex-row">
                    <h4>Select Date</h4>
                    <FaCalendarAlt className=" ms-4 fs-3" onClick={handleIconClick} />
                    <input
                        type='date'
                        ref={inputRef}
                ...
                <h4>Selected Date-: {date ? (`${date}`) : ('N/A')}</h4>
                ...
                <div className='container py-3 row'>
                    {filteredAndSearchedCompanies.length > 0 ? (
                        ...
                        <div className="text-center text-muted">No companies available with the selected groupage service filter.</div>
                    )}
                </div>
            </div>
